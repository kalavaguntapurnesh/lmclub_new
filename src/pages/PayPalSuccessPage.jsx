  ...
  return (
    <div className="fixed inset-0 flex justify-center items-center bg-gray-800 bg-opacity-50">
      <div className="bg-white w-[600px] p-6 rounded-lg shadow-lg ">
        ...
        {/* Loading State */}
        {loading && <p className="text-center text-gray-600">Processing payment...</p>}

        {/* Display Payment Details */}
        {paymentDetails && (
          <div className="text-gray-700">
             <p className="font-semibold text-3xl text-center text-green-500 mb-2"> Thank You!</p>
             <p className=" text-xl text-center mb-3"> Payment Done Successfully</p>
            ...
            <table className="w-full border border-gray-300 w-[80%]">
                ...
                <tr className="bg-gray-100">
                    ...
                    <td className="border px-4 py-2">{paymentDetails?.data?.id || "N/A"}</td>
                </tr>
                <tr className="bg-gray-300">
                    <td className="border px-4 py-2 font-semibold">Status:</td>
                    <td className="border px-4 py-2">{paymentDetails?.data?.status || "N/A"}</td>
                </tr>
                <tr className="bg-gray-100">
                    <td className="border px-4 py-2 font-semibold">Email:</td>
                    <td className="border px-4 py-2">{paymentDetails?.data?.payer?.email_address || "N/A"}</td>
                </tr>
                <tr className="bg-gray-300">
                    <td className="border px-4 py-2 font-semibold">Name:</td>
                    <td className="border px-4 py-2">{paymentDetails?.data?.payer?.name?.given_name || "N/A"}</td>
                </tr>
                <tr className="bg-gray-100">
                    <td className="border px-4 py-2 font-semibold">Amount:</td>
                    <td className="border px-4 py-2">
                    ${paymentDetails?.data?.purchase_units?.[0]?.payments?.captures?.[0]?.amount?.value || "N/A"}
                    </td>
                </tr>
                <tr className="bg-gray-300">
                    <td className="border px-4 py-2 font-semibold">Account ID:</td>
                    <td className="border px-4 py-2">{paymentDetails?.data?.payment_source?.paypal?.account_id || "N/A"}</td>
                </tr>
                </tbody>
            </table>
            </div>
        ...
        {!loading && !paymentDetails && (
          <p className="text-center text-red-500">Payment failed or details not found.</p>
        )}
